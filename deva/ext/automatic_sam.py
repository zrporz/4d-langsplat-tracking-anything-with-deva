...
def auto_segment(config: Dict, auto_sam: SamAutomaticMaskGenerator, image: np.ndarray,
    ...
    if forward_mask is not None:
        ...
        mask_data, curr_anns = auto_sam.generate(image, positive_points, negative_points, level=config['level'])
    else:
        mask_data, curr_anns  = auto_sam.generate(image, level=config['level'])
    # import ipdb; ipdb.set_trace()
    # 创建一个新的列表，用于存储转换后的数据
