...
class SamAutomaticMaskGenerator:
    ...
    def generate(self,
                 ...
                 level='default') -> List[Dict[str, Any]]:
        """
        ...
        # Generate masks
        mask_data  = self._generate_masks(image, positive_points, negative_points, level)
        # mask_data, mask_data_s, mask_data_m, mask_data_l  = self._generate_masks(image, positive_points, negative_points)
        ...
        if self.min_mask_region_area > 0:
            ...
            # mask_data_s = self.postprocess_small_regions(
            #     mask_data_s,
            #     self.min_mask_region_area,
            #     max(self.box_nms_thresh, self.crop_nms_thresh),
            # )
            # mask_data_m = self.postprocess_small_regions(
            #     mask_data_m,
            #     self.min_mask_region_area,
            #     max(self.box_nms_thresh, self.crop_nms_thresh),
            # )
            # mask_data_l = self.postprocess_small_regions(
            #     mask_data_l,
            #     self.min_mask_region_area,
            #     max(self.box_nms_thresh, self.crop_nms_thresh),
            # )
    ...
    def _generate_masks(self,
        ...
        for crop_box, layer_idx in zip(crop_boxes, layer_idxs):
            # crop_data, crop_data_s, crop_data_m, crop_data_l = self._process_crop(image, crop_box, layer_idx, orig_size, positive_points,
            #                                negative_points)
            crop_data = self._process_crop(image, crop_box, layer_idx, orig_size, positive_points,
                                           negative_points,level)
            data.cat(crop_data)
