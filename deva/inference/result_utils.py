...
class ResultSaver:
    def __init__(self,
                 ...
                 *,
                 ...
                 palette: Optional[ImagePalette.ImagePalette] = None, feature_level=None):
        ...
        if self.dataset == 'vipseg':
            ...
            self.json_style = 'vipseg'
            self.visualize = True
            self.visualize_postfix = f'Visualizations-{feature_level}'
            self.output_postfix = f'Annotations-{feature_level}'
    ...
    def save_mask(self,
                  ...
                  path_to_image: str = None,curr_anns=None):
...
def save_result(queue: Queue):
    while True:
        ...
        mask_path_root = path.join(saver.output_root, f"origin_mask_{saver.feature_level}")
        os.makedirs(mask_path_root,exist_ok=True)
        this_path = path.join(mask_path_root, frame_name[:-4])
        np.save(this_path, mask)
        # import ipdb; ipdb.set_trace()
